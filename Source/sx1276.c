...
void SX1276SetLoRaOn( BOOL enable )
{
    ...
    if( LoRaOn == TRUE )
    {
        SX1276LoRaSetOpMode( RFLR_OPMODE_SLEEP );								// 设置为Sleep mode
        
        SX1276LR->RegOpMode = ( SX1276LR->RegOpMode & RFLR_OPMODE_LONGRANGEMODE_MASK ) | RFLR_OPMODE_LONGRANGEMODE_ON;
        SX1276Write( REG_LR_OPMODE, SX1276LR->RegOpMode );			// 设置为LoRa模式（SX1276必须处于Sleep mode）
        
        SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY );							// 设置为Standby mode
                                        // RxDone               RxTimeout                   FhssChangeChannel           CadDone
        SX1276LR->RegDioMapping1 = RFLR_DIOMAPPING1_DIO0_00 | RFLR_DIOMAPPING1_DIO1_00 | RFLR_DIOMAPPING1_DIO2_00 | RFLR_DIOMAPPING1_DIO3_00;
                                        // CadDetected          ModeReady
        SX1276LR->RegDioMapping2 = RFLR_DIOMAPPING2_DIO4_00 | RFLR_DIOMAPPING2_DIO5_00;
        SX1276WriteBuffer( REG_LR_DIOMAPPING1, &SX1276LR->RegDioMapping1, 2 );			// DIO Mapping LoRa Mode
    ...
    else
    {
        ...
        SX1276Write( REG_LR_OPMODE, SX1276LR->RegOpMode );			// 设置为FSK模式
